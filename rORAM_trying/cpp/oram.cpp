...
ORAM::ORAM(int numBuckets, int bucketCapacity, const vector<unsigned char>& encryptionKey, int range_length, string file_name) {
    ...
    this->tree_file.open(file_name, std::ios::in | std::ios::out | std::ios::trunc);

    Bucket empty_bucket = Bucket();
    
    for (int i = 0; i < numBuckets; i++) {
        tree_file << serialize_bucket(empty_bucket);
    }
    tree_file.flush();
}

...
string serialize_bucket(Bucket bucket_to_serialize){
    ostringstream oss;
    for (block b : bucket_to_serialize.getBlocks()){
        oss << b.data;
    }
...
Bucket deserialize_bucket(string bucket_string){
    const size_t blockSize = 320;
    const size_t expectedSize = 1280;
    
    // Check that the input string is the expected size.
    if (bucket_string.size() != expectedSize) {
        cout << "read_string: " << bucket_string.size() << endl;
    ...
    for (size_t i = 0; i < expectedSize; i += blockSize) {
        string blockStr = bucket_string.substr(i, blockSize);
